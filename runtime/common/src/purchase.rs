...
//! Module to process purchase of PIRLS.
...
pub trait Config: frame_system::Config {
	...
	/// The amount of purchased locked PIRLS that we will unlock for basic actions on the chain.
	type UnlockedProportion: Get<Permill>;
	/// The maximum amount of locked PIRLS that we will unlock.
...
/// All information about an account regarding the purchase of PIRLS.
...
pub struct AccountStatus<Balance> {
	...
	/// The amount of free PIRLS they have purchased.
	free_balance: Balance,
	/// The amount of locked PIRLS they have purchased.
...
decl_storage! {
	trait Store for Module<T: Config> as Purchase {
		...
		Statement: Vec<u8>;
		// The block where all locked PIRLS will unlock.
...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		fn payout(origin, who: T::AccountId) {
			...
			Accounts::<T>::try_mutate(&who, |status: &mut AccountStatus<BalanceOf<T>>| -> DispatchResult {
				...
				if !status.locked_balance.is_zero() {
					...
					// We allow some configurable portion of the purchased locked PIRLS to be unlocked for basic usage.
		...
		/// Set the block where locked PIRLS will become unlocked.
		///
		/// Origin must match the `ConfigurationOrigin`
		#[weight = T::DbWeight::get().writes(1)]
